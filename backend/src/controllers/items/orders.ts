...
export const placeOrder: RequestHandler<unknown, unknown, OrderBody, unknown> = async (
  ...
  try {
    ...
    for (const key of itemsMap.keys()) {
      let item: Interfaces.MenuItem | null = null;
      if (!isMenuItem(itemsMap.get(key)!)) throw new Error("Menu is not a valid MenuItem");
      else item = itemsMap.get(key)! as Interfaces.MenuItem;
